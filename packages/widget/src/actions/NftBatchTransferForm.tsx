	...
	return (
		...
		>
			<div className="magi-token-tabs" style={{ marginBottom: '0.4rem' }}>
				<button
					type="button"
					className={`magi-token-tab ${mode === 'same' ? 'active' : ''}`}
			...
			>
				{rows.map((r, idx) => (
					<label
						key={r.tokenId}
						className={`magi-token-batch-row ${r.checked ? 'checked' : ''}`}
					>
						<input type="checkbox" checked={r.checked} onChange={() => toggle(idx)} />
						<span className="magi-token-batch-row-id">{r.tokenId}</span>
						{!r.isUnique && (
							<span className="magi-token-batch-row-balance">bal: {r.balance}</span>
						)}
				...
				{!rows.length && (
					<div className="magi-token-state">No tokens in this collection.</div>
				)}
			</div>
